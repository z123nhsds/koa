...
module.exports = {
  ...
  set type (type) {
    ...
    try {
      ...
      const normalized = getType(parsed.type)

      if (normalized) {
        const normalizedType = contentType.parse(normalized)
        parsed.type = normalizedType.type
        parsed.parameters = {
          ...normalizedType.parameters,
          ...parsed.parameters
        }
      }

      this.set('Content-Type', contentType.format(parsed))
    } catch {
      this.remove('Content-Type')
    }
  },
